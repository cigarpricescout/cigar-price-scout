...
class CigarCSVManager:
    ...
    def create_csv(self, retailer_key: str) -> bool:
        ...
        try:
            ...
            print(f"✗ Error creating {csv_path}: {e}")
            return False
    ...
    def add_cigar_data(self, retailer_key: str, cigar_data: Dict) -> bool:
        ...
        if retailer_key not in self.retailer_keys:
            print(f"✗ Unknown retailer key: {retailer_key}")
        ...
        try:
            ...
            for row in existing_data:
                if (row['brand'] == cigar_data['brand'] and 
                    ...
                    print(f"⚠ Duplicate found for {cigar_data['brand']} {cigar_data['line']} - updating price")
            ...
            print(f"✗ Error adding data to {csv_path}: {e}")
            return False
    
    def bulk_add_cigar(self, cigar_template: Dict, retailer_data: List[Dict]) -> None:
        """Add the same cigar to multiple retailers with different prices/URLs"""
        
        print(f"\n🚀 Adding {cigar_template['brand']} {cigar_template['line']} to multiple retailers...")
        
        success_count = 0
        for retailer_info in retailer_data:
            retailer_key = retailer_info['retailer_key']
            
            # Merge template with retailer-specific data
            cigar_data = cigar_template.copy()
            cigar_data.update(retailer_info)
            
            if self.add_cigar_data(retailer_key, cigar_data):
                success_count += 1
        
        print(f"✅ Successfully added to {success_count}/{len(retailer_data)} retailers\n")
    ...
    def process_romeo_julieta_data(self) -> bool:
        ...
        print("Processing Romeo y Julieta 1875 Churchill data...")
        ...
        for row in romeo_data:
            ...
            if not retailer_key:
                print(f"⚠ Unknown retailer: '{row['retailer']}' - skipping")
                skipped_count += 1
                continue
        ...
        print(f"✅ Successfully processed {processed_count} Romeo y Julieta entries")
        if skipped_count > 0:
