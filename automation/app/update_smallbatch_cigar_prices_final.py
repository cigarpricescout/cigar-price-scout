"""
Small Batch Cigar Enhanced CSV Updater with Master-Driven Metadata Sync
...
try:
    from smallbatch_cigar_extractor import extract_smallbatch_cigar_data
except ImportError:
    print("[ERROR] Could not import extract_smallbatch_cigar_data. Make sure the extractor is in tools/price_monitoring/retailers/smallbatch_cigar_extractor.py")
    sys.exit(1)


class SmallBatchCigarCSVUpdaterWithMaster:
    def __init__(self, csv_path: str = None, master_path: str = None, dry_run: bool = False):
        if csv_path is None:
            self.csv_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'data', 'smallbatchcigar.csv')
        else:
    ...
    def load_csv(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"[INFO] Loaded {len(data)} products from Small Batch Cigar CSV")
            return data
        except FileNotFoundError:
            print(f"[ERROR] Small Batch Cigar CSV not found at: {self.csv_path}")
            return []
        except Exception as e:
    ...
    def update_pricing_data(self, url: str) -> Dict:
        """Extract live pricing data from Small Batch Cigar"""
        try:
    ...
    def run_update(self) -> bool:
        ...
        print("=" * 70)
        print(f"{mode_str}SMALL BATCH CIGAR ENHANCED PRICE UPDATE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Small Batch Cigar price updater with master-driven metadata sync')
    parser.add_argument('--csv', help='Path to Small Batch Cigar CSV file')
    ...
    updater = SmallBatchCigarCSVUpdaterWithMaster(csv_path=args.csv, master_path=args.master, dry_run=dry_run)
    ...
    if success:
        print("\n[SUCCESS] Small Batch Cigar enhanced update completed successfully")
    else:
        print("\n[FAILED] Small Batch Cigar enhanced update failed")
        sys.exit(1)
