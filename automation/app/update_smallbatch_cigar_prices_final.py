"""
Small Batch Cigar CSV Updater with Google Sheets Master File Integration
Uses cigar_id from master_cigars.csv for metadata auto-population
Following the proven pattern with Shopify platform handling
"""
...
try:
    ...
    print(f"[ERROR] Could not import extract_smallbatch_cigar_data. Make sure smallbatch_cigar_extractor.py is in tools/price_monitoring/retailers/")
...
class SmallBatchCigarCSVUpdaterWithMaster:
    ...
    def run_update(self) -> bool:
        ...
        print("=" * 70)
        print(f"SMALL BATCH CIGAR PRICE UPDATE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("=" * 70)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Update Small Batch Cigar prices from CSV')
    ...
    updater = SmallBatchCigarCSVUpdaterWithMaster(csv_path=args.csv, master_path=args.master)
    ...
    if success:
        print("\n[SUCCESS] Small Batch Cigar price update completed successfully")
    else:
        print("\n[FAILED] Small Batch Cigar price update failed")
        sys.exit(1)
