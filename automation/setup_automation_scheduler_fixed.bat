...
powershell -Command "$t = Get-ScheduledTask 'CigarPriceScout_DailyAutomation'; $s = $t.Settings; $s.DisallowStartIfOnBatteries = $false; $s.StopIfGoingOnBatteries = $false; $s.StartWhenAvailable = $true; $s.WakeToRun = $true; Set-ScheduledTask -TaskName 'CigarPriceScout_DailyAutomation' -Settings $s | Out-Null"
