...
def opus_x_already_exists(csv_path):
    ...
    try:
        ...
        print(f"[ERROR] Error checking for existing OpusX in {csv_path}: {e}")
        return False
...
def add_opus_x_to_existing_csv(retailer_key, csv_path, opus_x_data):
    ...
    if opus_x_already_exists(csv_path):
        print(f"[SKIP] OpusX already exists in {retailer_key}")
        return True
    ...
    try:
        ...
        print(f"[SUCCESS] Added OpusX to existing {retailer_key}")
        return True
        
    except Exception as e:
        print(f"[ERROR] Error adding OpusX to existing {retailer_key}: {e}")
        return False
...
def create_new_csv(retailer_key, csv_path, opus_x_data):
    ...
    try:
        ...
        print(f"[SUCCESS] Created new CSV with OpusX for {retailer_key}")
        return True
        
    except Exception as e:
        print(f"[ERROR] Error creating CSV for {retailer_key}: {e}")
        return False

def main():
    print("==================================================================")
    print("COMPREHENSIVE OPUS X ROBUSTO CSV INTEGRATION")
    ...
    if not static_data_dir.exists():
        print(f"[INFO] Directory does not exist: {static_data_dir}")
        print("[INFO] Creating static/data directory...")
    ...
    if success_count == total_count:
        ...
        if new_files:
            ...
            for retailer in new_files:
                retailer_name = retailer.replace('cigars', ' Cigars').replace('cigar', ' Cigar').title()
                print(f'    {{"key": "{retailer}", "name": "{retailer_name}", "csv": "../static/data/{retailer}.csv", "authorized": False}},')
    else:
        print("\n*** WARNING: Some entries failed - check the errors above ***")
