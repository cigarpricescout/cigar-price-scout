"""
Cigar Cellar of Miami Enhanced CSV Updater with Master-Driven Metadata Sync
...
# Import the Cigar Cellar of Miami extractor
try:
    from retailers.cigarcellarofmiami_extractor import extract_cigarcellarofmiami_data
except ImportError:
    print("[ERROR] Could not import extract_cigarcellarofmiami_data. Make sure cigarcellarofmiami_extractor.py is in tools/price_monitoring/retailers/")
    sys.exit(1)


class CigarCellarOfMiamiCSVUpdater:
    def __init__(self, csv_path: str = None, master_path: str = None, dry_run: bool = False):
        if csv_path is None:
            self.csv_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'data', 'cigarcellarofmiami.csv')
        else:
    ...
    def load_csv(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"[INFO] Loaded {len(data)} products from Cigar Cellar of Miami CSV")
            return data
        except FileNotFoundError:
            print(f"[ERROR] Cigar Cellar of Miami CSV not found at: {self.csv_path}")
            return []
        except Exception as e:
    ...
    def update_pricing_data(self, url: str) -> Dict:
        """Extract live pricing data from Cigar Cellar of Miami"""
        try:
            result = extract_cigarcellarofmiami_data(url)
    ...
    def run_update(self) -> bool:
        ...
        print("=" * 70)
        print(f"{mode_str}CIGAR CELLAR OF MIAMI ENHANCED PRICE UPDATE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Cigar Cellar of Miami price updater with master-driven metadata sync')
    parser.add_argument('--csv', help='Path to Cigar Cellar of Miami CSV file')
    ...
    updater = CigarCellarOfMiamiCSVUpdater(csv_path=args.csv, master_path=args.master, dry_run=dry_run)
    ...
    if success:
        print("\n[SUCCESS] Cigar Cellar of Miami enhanced update completed successfully")
    else:
        print("\n[FAILED] Cigar Cellar of Miami enhanced update failed")
        sys.exit(1)
