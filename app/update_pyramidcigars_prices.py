"""
Pyramid Cigars Enhanced CSV Updater with Master-Driven Metadata Sync
...
# Import the Pyramid Cigars extractor
try:
    from retailers.pyramid_cigars_extractor import extract_pyramid_cigars_data
except ImportError:
    print("[ERROR] Could not import extract_pyramid_cigars_data. Make sure pyramid_cigars_extractor.py is in tools/price_monitoring/retailers/")
    sys.exit(1)


class PyramidCigarsCSVUpdater:
    def __init__(self, csv_path: str = None, master_path: str = None, dry_run: bool = False):
        if csv_path is None:
            self.csv_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'data', 'pyramidcigars.csv')
        else:
    ...
    def load_csv(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"[INFO] Loaded {len(data)} products from Pyramid Cigars CSV")
            return data
        except FileNotFoundError:
            print(f"[ERROR] Pyramid Cigars CSV not found at: {self.csv_path}")
            return []
        except Exception as e:
    ...
    def update_pricing_data(self, url: str) -> Dict:
        """Extract live pricing data from Pyramid Cigars"""
        try:
    ...
    def run_update(self) -> bool:
        ...
        print("=" * 70)
        print(f"{mode_str}PYRAMID CIGARS ENHANCED PRICE UPDATE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Pyramid Cigars price updater with master-driven metadata sync')
    parser.add_argument('--csv', help='Path to Pyramid Cigars CSV file')
    ...
    updater = PyramidCigarsCSVUpdater(csv_path=args.csv, master_path=args.master, dry_run=dry_run)
    ...
    if success:
        print("\n[SUCCESS] Pyramid Cigars enhanced update completed successfully")
    else:
        print("\n[FAILED] Pyramid Cigars enhanced update failed")
        sys.exit(1)
