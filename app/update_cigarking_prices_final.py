"""
Cigar King Enhanced CSV Updater with Master-Driven Metadata Sync
...
try:
    from cigar_king_extractor import CigarKingExtractor
except ImportError:
    print("[ERROR] Could not import CigarKingExtractor. Make sure the extractor is in tools/price_monitoring/retailers/cigar_king_extractor.py")
    sys.exit(1)


class CigarKingCSVUpdater:
    def __init__(self, csv_path: str = None, master_path: str = None, dry_run: bool = False):
        if csv_path is None:
            self.csv_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'data', 'cigarking.csv')
        else:
        ...
        self.extractor = CigarKingExtractor()
    ...
    def load_csv(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"[INFO] Loaded {len(data)} products from Cigar King CSV")
            return data
        except FileNotFoundError:
            print(f"[ERROR] Cigar King CSV not found at: {self.csv_path}")
            return []
        except Exception as e:
    ...
    def update_pricing_data(self, url: str) -> Dict:
        """Extract live pricing data from Cigar King"""
        try:
    ...
    def run_update(self) -> bool:
        ...
        print("=" * 70)
        print(f"{mode_str}CIGAR KING ENHANCED PRICE UPDATE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        for i, row in enumerate(data):
            ...
            if price_updated:
                price_str = f"${pricing_data['price']:.2f}"
            else:
                price_str = f"${row.get('price', 'N/A')} (historical)"
            
            stock_str = "In Stock" if pricing_data['in_stock'] else "Out of Stock"
            
            # Show original price if available (for sale detection)
            original_price_str = ""
            if pricing_data.get('original_price'):
                original_price_str = f" (was ${pricing_data['original_price']:.2f})"
            
            print(f"  [OK] {price_str}{original_price_str} | {stock_str}")
            successful_updates += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Cigar King price updater with master-driven metadata sync')
    parser.add_argument('--csv', help='Path to Cigar King CSV file')
    ...
    updater = CigarKingCSVUpdater(csv_path=args.csv, master_path=args.master, dry_run=dry_run)
    ...
    if success:
        print("\n[SUCCESS] Cigar King enhanced update completed successfully")
    else:
        print("\n[FAILED] Cigar King enhanced update failed")
        sys.exit(1)
