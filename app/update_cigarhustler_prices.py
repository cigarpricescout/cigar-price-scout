"""
Cigar Hustler Enhanced CSV Updater with Master-Driven Metadata Sync
...
# Import the Cigar Hustler extractor
try:
    from retailers.cigarhustler_extractor import extract_cigarhustler_data
except ImportError:
    print("[ERROR] Could not import extract_cigarhustler_data. Make sure cigarhustler_extractor.py is in tools/price_monitoring/retailers/")
    sys.exit(1)
...
class CigarHustlerCSVUpdater:
    ...
    def create_backup(self) -> bool:
        ...
        backup_name = f"cigarhustler_backup_{timestamp}.csv"
    ...
    def load_products(self) -> List[Dict]:
        ...
        try:
            with open(self.csv_path, 'r', encoding='utf-8', newline='') as f:
                ...
                for row in reader:
                    products.append(row)
            print(f"[INFO] Loaded {len(products)} products from Cigar Hustler CSV")
            return products
        except Exception as e:
    ...
    def update_product(self, product: Dict) -> Dict:
        ...
        try:
            result = extract_cigarhustler_data(url)
            ...
            if result['success']:
                ...
                product['current_promotions_applied'] = ''  # Cigar Hustler handles discounts internally
    ...
    def run(self) -> bool:
        """Main update process"""
        print("=" * 70)
        print(f"CIGAR HUSTLER ENHANCED PRICE UPDATE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    """Main entry point"""
    updater = CigarHustlerCSVUpdater()
    
    try:
        success = updater.run()
        if success:
            print("\n[SUCCESS] Cigar Hustler enhanced update completed successfully")
            sys.exit(0)
        else:
            print("\n[FAILED] Cigar Hustler update encountered errors")
            sys.exit(1)
