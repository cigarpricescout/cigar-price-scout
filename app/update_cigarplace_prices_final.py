"""
Cigar Place Enhanced CSV Updater with Master-Driven Metadata Sync
...
class CigarPlaceCSVUpdaterWithMaster:
    ...
    def load_csv(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"[INFO] Loaded {len(data)} products from Cigar Place CSV")
            return data
        except FileNotFoundError:
            print(f"[ERROR] Cigar Place CSV not found at: {self.csv_path}")
            return []
        except Exception as e:
    ...
    def update_pricing_data(self, url: str) -> Dict:
        """Extract live pricing data from Cigar Place"""
        try:
    ...
    def run_update(self) -> bool:
        ...
        print("=" * 70)
        print(f"{mode_str}CIGAR PLACE ENHANCED PRICE UPDATE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Cigar Place price updater with master-driven metadata sync')
    parser.add_argument('--csv', help='Path to Cigar Place CSV file')
    ...
    updater = CigarPlaceCSVUpdaterWithMaster(csv_path=args.csv, master_path=args.master, dry_run=dry_run)
    ...
    if success:
        print("\n[SUCCESS] Cigar Place enhanced update completed successfully")
    else:
        print("\n[FAILED] Cigar Place enhanced update failed")
        sys.exit(1)
