...
try:
    from neptune_cigar_extractor import extract_neptune_cigar_data
except ImportError:
    print("[ERROR] Could not import extract_neptune_cigar_data. Make sure the extractor is in tools/price_monitoring/retailers/neptune_cigar_extractor.py")
    sys.exit(1)
...
class NeptuneCSVUpdaterWithMaster:
    ...
    def load_csv(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"[INFO] Loaded {len(data)} products from Neptune CSV")
            return data
        except FileNotFoundError:
            print(f"[ERROR] Neptune CSV not found at: {self.csv_path}")
            return []
        except Exception as e:
