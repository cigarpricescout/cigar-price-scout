"""
Cigar Prime Store Enhanced CSV Updater with Master-Driven Metadata Sync
...
# Import the Cigar Prime Store extractor
try:
    from retailers.cigarprimestore_extractor import extract_cigarprimestore_data
except ImportError:
    print("[ERROR] Could not import extract_cigarprimestore_data. Make sure cigarprimestore_extractor.py is in tools/price_monitoring/retailers/")
    sys.exit(1)


class CigarPrimeStoreCSVUpdater:
    def __init__(self, csv_path: str = None, master_path: str = None, dry_run: bool = False):
        if csv_path is None:
            self.csv_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'data', 'cigarprimestore.csv')
        else:
    ...
    def create_backup(self) -> bool:
        ...
        backup_name = f"cigarprimestore_backup_{timestamp}.csv"
    ...
    def load_products(self) -> List[Dict]:
        ...
        try:
            with open(self.csv_path, 'r', encoding='utf-8', newline='') as f:
                ...
                for row in reader:
                    products.append(row)
            print(f"[INFO] Loaded {len(products)} products from Cigar Prime Store CSV")
            return products
        except Exception as e:
    ...
    def update_product(self, product: Dict) -> Dict:
        ...
        try:
            result = extract_cigarprimestore_data(url)
    ...
    def run(self) -> bool:
        """Main update process"""
        print("=" * 70)
        print(f"CIGAR PRIME STORE ENHANCED PRICE UPDATE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    """Main entry point"""
    updater = CigarPrimeStoreCSVUpdater()
    
    try:
        success = updater.run()
        if success:
            print("\n[SUCCESS] Cigar Prime Store enhanced update completed successfully")
            sys.exit(0)
        else:
            print("\n[FAILED] Cigar Prime Store update encountered errors")
            sys.exit(1)
