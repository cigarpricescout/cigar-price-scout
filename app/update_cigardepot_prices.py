"""
Cigar Depot Enhanced CSV Updater with Master-Driven Metadata Sync
...
# Import the Cigar Depot extractor
try:
    from retailers.cigardepot_extractor import extract_cigardepot_data
except ImportError:
    print("[ERROR] Could not import extract_cigardepot_data. Make sure cigardepot_extractor.py is in tools/price_monitoring/retailers/")
    sys.exit(1)


class CigarDepotCSVUpdater:
    def __init__(self, csv_path: str = None, master_path: str = None, dry_run: bool = False):
        if csv_path is None:
            self.csv_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'data', 'cigardepot.csv')
        else:
    ...
    def load_csv(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"[INFO] Loaded {len(data)} products from Cigar Depot CSV")
            return data
        except FileNotFoundError:
            print(f"[ERROR] Cigar Depot CSV not found at: {self.csv_path}")
            return []
        except Exception as e:
    ...
    def update_pricing_data(self, url: str) -> Dict:
        """Extract live pricing data from Cigar Depot"""
        try:
            result = extract_cigardepot_data(url)
    ...
    def run_update(self) -> bool:
        ...
        print("=" * 70)
        print(f"{mode_str}CIGAR DEPOT ENHANCED PRICE UPDATE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Cigar Depot price updater with master-driven metadata sync')
    parser.add_argument('--csv', help='Path to Cigar Depot CSV file')
    ...
    updater = CigarDepotCSVUpdater(csv_path=args.csv, master_path=args.master, dry_run=dry_run)
    ...
    if success:
        print("\n[SUCCESS] Cigar Depot enhanced update completed successfully")
    else:
        print("\n[FAILED] Cigar Depot enhanced update failed")
        sys.exit(1)
