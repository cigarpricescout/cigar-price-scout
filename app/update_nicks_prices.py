"""
Nick\'s Cigar World Enhanced CSV Updater with Master-Driven Metadata Sync
...
class NicksCigarWorldCSVUpdaterWithMaster:
    ...
    def load_csv(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"[INFO] Loaded {len(data)} products from Nick\'s Cigar World CSV")
            return data
        except FileNotFoundError:
            print(f"[ERROR] Nick\'s Cigar World CSV not found at: {self.csv_path}")
            return []
        except Exception as e:
    ...
    def update_pricing_data(self, url: str) -> Dict:
        """Extract live pricing data from Nick\'s Cigar World"""
        try:
    ...
    def run_update(self) -> bool:
        ...
        print("=" * 70)
        print(f"{mode_str}NICK'S CIGAR WORLD ENHANCED PRICE UPDATE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Nick\'s Cigar World price updater with master-driven metadata sync')
    parser.add_argument('--csv', help='Path to Nick\'s Cigar World CSV file')
    ...
    updater = NicksCigarWorldCSVUpdaterWithMaster(csv_path=args.csv, master_path=args.master, dry_run=dry_run)
    ...
    if success:
        print("\n[SUCCESS] Nick\'s Cigar World enhanced update completed successfully")
    else:
        print("\n[FAILED] Nick\'s Cigar World enhanced update failed")
        sys.exit(1)
