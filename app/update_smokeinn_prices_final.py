#!/usr/bin/env python3
"""
Smoke Inn Enhanced Price Updater with Master-Driven Metadata Sync  
...
def update_smokeinn_prices(dry_run=False):
    """Enhanced Smoke Inn pricing update with master-driven metadata sync"""
    mode_str = "[DRY RUN] " if dry_run else ""
    print("=" * 70)
    print(f"{mode_str}SMOKE INN ENHANCED PRICE UPDATE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    try:
        smokeinn_df = pd.read_csv(csv_path)
        print(f"[INFO] Loaded {len(smokeinn_df)} products from Smoke Inn CSV")
    except Exception as e:
    ...
    for index, row in smokeinn_df.iterrows():
        ...
        print(f"\n[{product_num}/{len(smokeinn_df)}] Processing: {cigar_id}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Smoke Inn price updater with master-driven metadata sync')
    ...
    try:
        success = update_smokeinn_prices(dry_run=dry_run)
        if not success:
            sys.exit(1)
    except Exception as e:
