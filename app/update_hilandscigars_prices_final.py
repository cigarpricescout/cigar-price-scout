"""
Hiland's Cigars Enhanced CSV Updater with Master-Driven Metadata Sync
...
# Add the retailers directory directly to path
retailers_dir = os.path.join(os.path.dirname(__file__), '..', 'tools', 'price_monitoring', 'retailers')
sys.path.append(retailers_dir)

try:
    # Direct import from the hilands_cigars.py file
    import hilands_cigars
    extract_hilands_cigars_data = hilands_cigars.extract_hilands_cigars_data
except ImportError as e:
    print(f"[ERROR] Could not import extract_hilands_cigars_data. Make sure hilands_cigars.py is in tools/price_monitoring/retailers/")
    print(f"[ERROR] Import error details: {e}")
    print(f"[ERROR] Looking for file at: {os.path.join(retailers_dir, 'hilands_cigars.py')}")
    print(f"[ERROR] File exists: {os.path.exists(os.path.join(retailers_dir, 'hilands_cigars.py'))}")
    sys.exit(1)


class HilandsCSVUpdaterWithMaster:
    def __init__(self, csv_path: str = None, master_path: str = None, dry_run: bool = False):
        if csv_path is None:
            self.csv_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'data', 'hilands.csv')
        else:
    ...
    def load_csv(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"[INFO] Loaded {len(data)} products from Hiland's CSV")
            return data
        except FileNotFoundError:
            print(f"[ERROR] Hiland's CSV not found at: {self.csv_path}")
            return []
        except Exception as e:
    ...
    def update_pricing_data(self, url: str) -> Dict:
        """Extract live pricing data from Hiland's Cigars"""
        try:
    ...
    def run_update(self) -> bool:
        ...
        print("=" * 70)
        print(f"{mode_str}HILAND'S CIGARS ENHANCED PRICE UPDATE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Hiland\'s Cigars price updater with master-driven metadata sync')
    parser.add_argument('--csv', help='Path to Hiland\'s CSV file')
    ...
    updater = HilandsCSVUpdaterWithMaster(csv_path=args.csv, master_path=args.master, dry_run=dry_run)
    ...
    if success:
        print("\n[SUCCESS] Hiland's Cigars enhanced update completed successfully")
    else:
        print("\n[FAILED] Hiland's Cigars enhanced update failed")
        sys.exit(1)
