"""
Big Humidor Enhanced CSV Updater with Master-Driven Metadata Sync
...
try:
    from big_humidor_extractor import BigHumidorExtractor
except ImportError:
    print("[ERROR] Could not import BigHumidorExtractor. Make sure the extractor is in tools/price_monitoring/retailers/big_humidor_extractor.py")
    sys.exit(1)


class BigHumidorCSVUpdater:
    def __init__(self, csv_path: str = None, master_path: str = None, dry_run: bool = False):
        if csv_path is None:
            self.csv_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'data', 'bighumidor.csv')
        else:
        ...
        self.extractor = BigHumidorExtractor()
    ...
    def load_csv(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"[INFO] Loaded {len(data)} products from Big Humidor CSV")
            return data
        except FileNotFoundError:
            print(f"[ERROR] Big Humidor CSV not found at: {self.csv_path}")
            return []
        except Exception as e:
    ...
    def update_pricing_data(self, url: str) -> Dict:
        """Extract live pricing data from Big Humidor"""
        try:
    ...
    def run_update(self) -> bool:
        ...
        print("=" * 70)
        print(f"{mode_str}BIG HUMIDOR ENHANCED PRICE UPDATE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Big Humidor price updater with master-driven metadata sync')
    parser.add_argument('--csv', help='Path to Big Humidor CSV file')
    ...
    updater = BigHumidorCSVUpdater(csv_path=args.csv, master_path=args.master, dry_run=dry_run)
    ...
    if success:
        print("\n[SUCCESS] Big Humidor enhanced update completed successfully")
    else:
        print("\n[FAILED] Big Humidor enhanced update failed")
        sys.exit(1)
