#!/usr/bin/env python3
"""
TheCigarShop Price Updater
Updates pricing data for TheCigarShop products using master-driven metadata sync approach

Usage:
    python update_thecigarshop_prices.py [--dry-run]
...
try:
    from retailers.thecigarshop_extractor import extract_thecigarshop_data
except ImportError as e:
    print(f"Error importing TheCigarShop extractor: {e}")
    print("Make sure thecigarshop_extractor.py is in tools/price_monitoring/retailers/")
    sys.exit(1)

class TheCigarShopPriceUpdater:
    def __init__(self, dry_run=False):
        self.dry_run = dry_run
        self.project_root = project_root
        self.csv_path = self.project_root / "static" / "data" / "thecigarshop.csv"
    ...
    def extract_product_data(self, url):
        """Extract product data from TheCigarShop URL with error handling"""
        try:
    ...
    def update_prices(self):
        ...
        print("=== TheCigarShop Enhanced Price Update Process ===")
    ...
    def load_current_data(self):
        """Load current TheCigarShop CSV data"""
        current_data = {}
        ...
        try:
            ...
            print(f"Loaded {len(current_data)} existing TheCigarShop products")
...
def main():
    parser = argparse.ArgumentParser(description='Update TheCigarShop pricing data')
    parser.add_argument('--dry-run', action='store_true', 
                       help='Show changes without updating files')
    
    args = parser.parse_args()
    
    updater = TheCigarShopPriceUpdater(dry_run=args.dry_run)
