"""
Stogies World Class Cigars Enhanced CSV Updater with Master-Driven Metadata Sync
...
try:
    from stogies_extractor import extract_stogies_data
except ImportError:
    print("[ERROR] Could not import extract_stogies_data. Make sure the extractor is in tools/price_monitoring/retailers/stogies_extractor.py")
    sys.exit(1)


class StogiesCSVUpdaterWithMaster:
    def __init__(self, csv_path: str = None, master_path: str = None, dry_run: bool = False):
        if csv_path is None:
            self.csv_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'data', 'stogies.csv')
        else:
    ...
    def load_csv(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"[INFO] Loaded {len(data)} products from Stogies World Class CSV")
            return data
        except FileNotFoundError:
            print(f"[ERROR] Stogies World Class CSV not found at: {self.csv_path}")
            return []
        except Exception as e:
    ...
    def save_csv(self, data: List[Dict]) -> bool:
        ...
        try:
            fieldnames = list(data[0].keys()) if data else ['cigar_id', 'title', 'url', 'brand', 'line', 'wrapper', 'vitola', 'size', 'box_qty', 'price', 'in_stock', 'current_promotions_applied']
    ...
    def update_pricing_data(self, url: str, cigar_id: str) -> Dict:
        """Extract live pricing data from Stogies World Class"""
        try:
            # Call the extractor
            result = extract_stogies_data(url, cigar_id)
    ...
    def run_update(self) -> bool:
        ...
        print("=" * 70)
        print(f"{mode_str}STOGIES WORLD CLASS ENHANCED PRICE UPDATE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        for i, row in enumerate(data):
            ...
            retail_str = f" (Retail: ${pricing_data['retail_price']})" if pricing_data.get('retail_price') else ""
            
            print(f"  [OK] {price_str} | {stock_str}{retail_str}")
            successful_updates += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Stogies World Class price updater with master-driven metadata sync')
    parser.add_argument('--csv', help='Path to Stogies World Class CSV file')
    ...
    updater = StogiesCSVUpdaterWithMaster(csv_path=args.csv, master_path=args.master, dry_run=dry_run)
    ...
    if success:
        print("\n[SUCCESS] Stogies World Class enhanced update completed successfully")
    else:
        print("\n[FAILED] Stogies World Class enhanced update failed")
        sys.exit(1)
